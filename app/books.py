...
import hashlib
from app import db
from app.models import Book, Genre, Review, Cover
...
def detail(book_id):
    ...
    user_collections = []
    if current_user.is_authenticated and hasattr(current_user, 'role') and current_user.role.name == 'user':
        from app.models import Collection
        user_collections = Collection.query.filter_by(user_id=current_user.id).all()
        for collection in user_collections:
            collection.is_in = book in collection.books

    from app.utils import md_to_html
    book.description_html = md_to_html(book.description)

    return render_template('books/detail.html', book=book, user_collections=user_collections)
